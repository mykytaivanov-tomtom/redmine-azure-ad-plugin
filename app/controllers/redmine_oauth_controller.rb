...
class RedmineOauthController < AccountController
  ...
  def https_callback_url
    oauth_azure_callback_url(protocol: 'https', host: request.host, port: 8080)
  end

  private

  # Override redirect_to to force HTTP protocol to avoid Mixed Content issues
  def redirect_to(options = {}, response_options = {})
    if options.is_a?(String)
      # If it's a string URL, convert to HTTP
      url = options
      unless url.start_with?('http://', 'https://')
        # Relative URL - add host
        host = request.host_with_port
        url = "http://#{host}#{url.start_with?('/') ? url : '/' + url}"
      else
        # Absolute URL - force HTTP
        url = url.gsub(/^https:/, 'http:')
      end
      super(url, response_options)
    elsif options.is_a?(Hash)
      # Add protocol: 'http' to hash options
      options = options.merge(protocol: 'http')
